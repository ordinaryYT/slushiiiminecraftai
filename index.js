...
async function postOrUpdateGrassMessage(channel) {
  ...
  const users = Number(stats.rows[0]?.users || 0);
  if (total === lastTotal && users === lastUsers) return;
  lastTotal = total;
  lastUsers = users;
  ...
  const embed = new EmbedBuilder()
    .setDescription(\`🌿 **\${total.toLocaleString()}** grass touched by **\${users.toLocaleString()}** people.\n\n*Last <t:\${Math.floor(Date.now() / 1000)}:R>*\n\n🔊 **\${activeUsers}** people are in voice chat — join them to touch grass automatically!\`)
    .setColor(0x57F287)
    .setFooter({ text: 'New things coming soon 😄' });
  ...
  try {
    if (lastGrassMessageId) {
      const msg = await channel.messages.fetch(lastGrassMessageId);
...
// Append to existing interactionCreate handler
client.on('interactionCreate', async interaction => {
  if (interaction.isButton()) {
    ...
    if (customId === 'manual_grass') {
      await db.query(\`
        ...
        VALUES ($1, $2, 15)
        ON CONFLICT (user_id)
        DO UPDATE SET total_grass = grass_stats.total_grass + 15, last_touch = NOW()
...
// Append to existing voiceStateUpdate
...
client.on('voiceStateUpdate', async (oldState, newState) => {
  ...
  if (left) {
    ...
    if (session) {
      ...
      const fullChunks = Math.floor(duration / 30);
      if (fullChunks >= 1) {
        const multiplier = (!session.muted && !session.deafened) ? 2 : 1;
        const grass = fullChunks * 15 * multiplier;
        await db.query(\`
          INSERT INTO grass_stats (user_id, username, total_grass)
          VALUES ($1, $2, $3)
          ON CONFLICT (user_id)
...
// Append to existing ready handler
client.once('ready', async () => {
